...
def extract_tables_with_dates(soup):
    table_data = []
    h3_tags = soup.find_all('h3', class_='tit1')

    for h3 in h3_tags:
        h3_text = h3.text.strip()
        date = extract_date_from_caption(h3_text)
...
def extract_table_data(tax_type, period, date, table):
    table_data = []
...
def get_existing_vat_data(vector_store):
    """
    기존의 부가가치세 데이터 가져오기
    """
    return vector_store.search_in_partition(query="", partition_name="VATInfo", k=1000)
...
def get_relevant_data(all_tax_data, query_date, cutoff_data):
...
def main(query_date=None):
    ...
    tables_with_dates = extract_tables_with_dates(soup)
    ...
    for tax_type, period, date, table in tables_with_dates:
        table_data = extract_table_data(tax_type, period, date, table)
    ...
    # 기존 데이터 불러오기
    existing_data = get_existing_vat_data(vector_store)
    ...
    if query_date:
        tax_data_for_date = get_relevant_data(all_tax_data, query_date, max([d for t, p, d, _ in tables_with_dates if d]))
    else:
