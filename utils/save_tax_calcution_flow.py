...
def get_existing_data(vector_store, partition_name):
    """
    기존의 데이터를 조회
    """
    return vector_store.search_in_partition(query="", partition_name=partition_name, k=1000)

def are_data_equal(new_data, existing_data):
    """
...
def save_tax_flow_to_vectordb(tax_flow_dict):
    try:
        ...
        # 기존 데이터 가져오기
        existing_data = get_existing_data(vector_store, partition_name)

        # 데이터 비교 및 업데이트 로직
        if existing_data : 
            if are_data_equal(content, existing_data):
...
def main():
    ...
    save_tax_flow_to_vectordb(tax_flow_dict["content"])
