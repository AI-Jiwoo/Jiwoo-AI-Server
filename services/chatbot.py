...
class Chatbot:
    ...
    def _prepare_context(self, relevant_info: List[Dict[str, str]]) -> str:
        ...
        for info in relevant_info:
            part = f"[제목: {info['title']}]\6666666666666666666n{info['snippet']}"
            if info['url']:
                part += f"\n[출처: {info['url']}]"
            if info['date']:
                part += f"\n[날짜: {info['date']}]"
...
class TaxationChatbot(Chatbot):
    """세무 관련 질문에 답변하는 챗봇 클래스"""
    ...
    def get_taxation_data(self, business_id: int) -> Dict[str, List[Dict[str, str]]]:
        """특정 business_id에 해당하는 데이터를 가져옴"""
        return self.vector_store.get_data_by_business_id(business_id)
    
    def _get_taxation_relevant_info(self, user_input : str, intent : str) -> List[Dict[str, str]]:
        """세무 관련 정보를 벡터 스토어 또는 웹 검색을 통해 가져옴"""
        ...
        if relevant_info:
            logger.info(f"벡터 스토어에서 관련 정보를 찾았습니다. 결과 수: {len(relevant_info)}")
             ...
             # title 필드를 추가
            for info in relevant_info:
                snippet = info.get('snippet', '해당 정보 없음')
                info['title'] = info['content'].split('\n')[0]  # 첫 번째 줄을 title로 사용
    ...
    def get_taxation_response(self, business_id: int, user_input: str) -> str :
        """세무 관련 질문에 대한 응답 생성"""
        try:
            # 사업자 데이터 가져옴
            ...
            # 세무 관련 정보 검색 (businessId 사용)
            relevant_info = self._get_taxation_relevant_info(user_input, intent)

            logger.info(f"relevant_info에 넣기전 taxation_data : {taxation_data}")


            relevant_info.extend([
                {**item, 'title': 'Business Info'} for item in taxation_data.get("BusinessInfo", [])
            ])
            relevant_info.extend([
                {**item, 'title': 'Question Info'} for item in taxation_data.get("Question", [])
            ])
            relevant_info.extend([
                {**item, 'title': 'Transaction Info'} for item in taxation_data.get("TransactionFile", [])
            ])
            relevant_info.extend([
                {**item, 'title': 'Income Tax Info'} for item in taxation_data.get("IncomeTaxFile", [])
            ])
            ...
            logger.error(f"키 오류 발생: {str(ke)}")
            return "데이터 구조에 문제가 있습니다. 잠시 후 다시 시도해주세요."
        
        except Exception as e :
            logger.error(f"응답 생성 중 오류 발생 : {str(e)}")
            return "요청을 처리하는 동안 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
